import Mccalum.Puiseux.Conclusion2Factor

/-!
# Conclusion 2 — general codimension `e`, reduced to the e≥2 blow-up
...
(`order_eval_value_e1_weierstrass`), and `e ≥ 2` via the single remaining temporary axiom
`order_eval_value_e2_blowup` (the order-under-blow-up reduction, scoped in the memory). Wiring this into
`zariski_order_invariant_in_graph` replaces that broad axiom with the much more specific blow-up one.
-/
...
/-- **TEMPORARY AXIOM — the `e ≥ 2` order-under-blow-up reduction.** The exact analogue of
...
root `ψ`, the evaluation order is constant along the section. To be discharged by the quadratic blow-up
to codimension one (mirror `ZariskiE2`, tracking the multivariate order through the degenerate map). -/
axiom order_eval_value_e2_blowup {s k : ℕ} (fac : CParam s (k + 2) → Polynomial ℂ) (d : ℕ) (hd : 2 ≤ d)

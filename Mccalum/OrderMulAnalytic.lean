...
Relocated from `Mccalum.Generalized.Lifting` so the analytic germ ring `𝒪ₙ`
(`Mccalum.Generalized.AnalyticGerm`) can use it as the valuation/additivity engine without

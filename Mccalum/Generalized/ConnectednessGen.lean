...
The `Fin 1 → ℂ` connectedness kernel (`Mccalum.Puiseux.RootCover`) extends factor coefficients across
...
committed `n = 0` results (`Mccalum.Generalized.Monodromy`) remain valid instances.
-/

...
/-- **Differentiate under a parametric circle integral.** -/
theorem circleIntegral_hasDerivAt
    ...
    HasDerivAt (fun z => ∮ ζ in C(c, R), Φ z ζ) (∮ ζ in C(c, R), Φ' z₀ ζ) z₀ := by
  set K : Set (ℂ × ℂ) := closedBall z₀ δ ×ˢ sphere c R with hK
  ...
  have hmemK : ∀ z ∈ ball z₀ δ, ∀ θ : ℝ, ((z, circleMap c R θ) : ℂ × ℂ) ∈ K :=
    fun z hz θ => ⟨ball_subset_closedBall hz, circleMap_mem_sphere c hR θ⟩
  have hmemK0 : ∀ θ : ℝ, ((z₀, circleMap c R θ) : ℂ × ℂ) ∈ K :=
    fun θ => ⟨mem_closedBall_self hδ.le, circleMap_mem_sphere c hR θ⟩
  -- slice continuity (for measurability / integrability)
  ...
  have hslice' : ∀ z ∈ ball z₀ δ, Continuous (fun θ : ℝ => Φ' z (circleMap c R θ)) := by
    ...
    have hc : ContinuousOn (fun ζ : ℂ => Φ' z ζ) (sphere c R) :=
      hΦ'.comp (continuous_const.prodMk continuous_id).continuousOn
        (fun ζ hζ => ⟨ball_subset_closedBall hz, hζ⟩)
    exact hc.comp_continuous (continuous_circleMap c R) (fun θ => circleMap_mem_sphere c hR θ)
  set F : ℂ → ℝ → ℂ := fun z θ => deriv (circleMap c R) θ • Φ z (circleMap c R θ) with hF
  set F' : ℂ → ℝ → ℂ := fun z θ => deriv (circleMap c R) θ • Φ' z (circleMap c R θ) with hF'
  ...
  -- apply the parametric-derivative lemma
  have key := intervalIntegral.hasDerivAt_integral_of_dominated_loc_of_deriv_le
    ...
    (a := 0) (b := 2 * π)
    (ball_mem_nhds z₀ hδ)
    (eventually_of_mem (ball_mem_nhds z₀ hδ)
      (fun z hz => (hFcont z hz).aestronglyMeasurable))
    ...
    (ae_of_all _ fun θ _ z hz => by
      have hnorm : ‖F' z θ‖ = R * ‖Φ' z (circleMap c R θ)‖ := by
        ...
        simp only [norm_smul, deriv_circleMap, norm_mul, norm_circleMap_zero, norm_I, mul_one,
          abs_of_nonneg hR]
      rw [hnorm]
      have : ‖Φ' z (circleMap c R θ)‖ ≤ M := le_trans (le_abs_self _)
        (by simpa [Real.norm_eq_abs] using hM _ (hmemK z hz θ))
...
theorem circleIntegral_differentiableOn
  ...
  have hd := circleIntegral_hasDerivAt hδ'pos hR
    (hΦ.mono (Set.prod_mono hsubc le_rfl)) (hΦ'.mono (Set.prod_mono hsubc le_rfl))
